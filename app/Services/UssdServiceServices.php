...
class UssdServiceServices
{
     ...
     */
    function ussd_service_report()
    {
        $list = array();

        try {
            $records_list = \DB::table('ussd.menu_ussd_service')
            ->orderBy('id', 'asc')
            ->get();
    ...
    function ussd_service_get_info($id)
    {
        ...
        try {
            $menu_ussd_operator = \DB::table('ussd.menu_ussd_service')
                ->where('id', $id)
                ->get();
    ...
    function ussd_service_get_by_description($description)
    {
        $message = 'Service obtenido correctamente.';
        $error = false;
        $error_detail = null;
        $data = null;
        ...
        try {
            ...
            if (count($menu_ussd_service) > 0) {
                $menu_ussd_service[0]->status = ($menu_ussd_service[0]->status) ? 'Activo' : 'Inactivo';
                $data = $menu_ussd_service[0];
            }
        } catch (\Exception $e) {
            $error_detail = $this->custom_error($e, __FUNCTION__);
            $message = 'Ocurrió un problema al obtener el registro';
    ...
    function ussd_service_set_status($request, $user_id)
    {
        ...
        try {
            ...
            \DB::table('ussd.menu_ussd_service')
                ->where('id', $id)
                ->update([
                    'status' => $status,
                    'user_id' => $user_id,
